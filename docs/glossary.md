...
- **Run** (`PipelineRun`) - one row per nightly pipeline execution: start/end time, duration, ok/fail, and per source `{fetched, queued, error}`. The daily mail (`pipeline/report.py`) reads the latest one for liveness. Per-publisher history lives on the Publisher row instead: `last_fetched_at` (its feed last answered), `last_new_at` (it last gave a new candidate), `last_error` (its last fetch failed, cleared by the next good one).
