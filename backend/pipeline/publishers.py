...
- map a fetcher's ``source_type`` string -> the ``Channel`` enum (deterministic).
"""
...
# ─── channel mapping (NOT an LLM call — set deterministically) ──────────────
# Fetchers stamp one of these source_type strings; map to the Channel enum.
CHANNEL_BY_SOURCE_TYPE: dict[str, Channel] = {
    "hackerNews": Channel.hackernews,
    "aiNews": Channel.ainews,
    "newsletter": Channel.newsletter,
    "rss": Channel.rss,
}


def channel_for(source_type: str) -> Channel:
    """source_type -> Channel. Unknown types fall back to rss (with a warning)."""
    channel = CHANNEL_BY_SOURCE_TYPE.get(source_type)
    if channel is None:
        log(f"  !! unknown source_type {source_type!r}; defaulting channel=rss")
        return Channel.rss
    return channel
