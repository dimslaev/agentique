...
class SourceStats:
    ...
    # News / AI News / Newsletter / Feeds), which maps to a Channel now, not a
    # publisher. Fine as a stats grouping key, but if per-publisher health is
    # wanted, group by publisher_id/channel instead. PROBE_URL_BY_SOURCE stays
    # keyed by the aggregator labels. Left as-is (non-trivial) — see REFACTOR_NOTES.md.
    source: str
    fetched: int = 0
