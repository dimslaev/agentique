...
- `.github/workflows/prod-sql.yml` (`workflow_dispatch`) runs read/write SQL on
  the box without opening a port: the self-hosted runner pipes the statement
  into `agentique-sql`, which uses a role-scoped `psql` (read-only vs. DML, each
  with its own timeouts). Every run is an Actions entry with the statement,
  output, and who dispatched it.
- Postgres is published on loopback only and the SQL roles are passwordless, so
  they can only be reached through the local socket (`docker exec`), never over
  the network.
